...
to setup
  ...
  if export-replay? [ export-replay-frame ]
end
...
to export-replay-frame
  ;; OBSERVER-ONLY web-replay export. Runs only when the export-replay? switch is ON
  ;; (OFF in every published experiment -> bit-identical). RNG isolated via
  ;; with-local-randomness so even the replay run's dynamics are a faithful trajectory.
  let nf (word "data/Replay/" scenario-type "_nodes.csv")
  let ef (word "data/Replay/" scenario-type "_edges.csv")
  if current-week = 0 [
    carefully [ if file-exists? nf [ file-delete nf ] ] [ ]
    carefully [ if file-exists? ef [ file-delete ef ] ] [ ]
    file-open nf  file-print "week,who,kind,x,y,mot,state"  file-close
    file-open ef  file-print "week,e1,e2,cross"  file-close
  ]
  with-local-randomness [
    file-open nf
    ask turtles [
      let kind (ifelse-value is-refugee? self ["migrant"] [ ifelse-value is-local? self ["local"] [ ifelse-value is-trainer? self ["trainer"] ["park"] ] ])
      let astate "active"
      let mot 0
      if (kind = "migrant" or kind = "local") [
        set mot motivation
        if winter-paused? [ set astate "paused" ]
        if has-dropped-out? [ set astate "dropped" ]
      ]
      file-print (word current-week "," who "," kind "," precision xcor 2 "," precision ycor 2 "," precision mot 3 "," astate)
    ]
    file-close
    file-open ef
    ask friendships [ file-print (word current-week "," [who] of end1 "," [who] of end2 "," (ifelse-value is-cross-group? ["1"] ["0"])) ]
    file-close
  ]
end
...
export-edge-list</postRun>
  ...
  <experiment name="Replay_Baseline" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>retention-rate-percent</metric>
    <enumeratedValueSet variable="export-replay?"><value value="true"/></enumeratedValueSet>
    <enumeratedValueSet variable="scenario-type"><value value="&quot;Baseline&quot;"/></enumeratedValueSet>
    <enumeratedValueSet variable="num-parks"><value value="5"/></enumeratedValueSet>
    <enumeratedValueSet variable="refugees-per-group"><value value="15"/></enumeratedValueSet>
    <enumeratedValueSet variable="locals-per-group"><value value="5"/></enumeratedValueSet>
