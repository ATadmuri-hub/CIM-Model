;; ============================================================================
;; CALISTHENICS INTEGRATION MODEL (CIM) v6.3
;; Agent-Based Model for Sport-Driven Migrant Integration
;;
;; Aligns with: Thesis v6.3
...
;; - 16 policy scenarios + 1 auxiliary (100-500 runs each)
...
;; [FIX 2] Language learning uses logistic ceiling factor → diminishing returns near C2
...
globals [
  ...
  param-peer-influence-coef             ;; 0.08 (Bianchi 2015)
  ...
  param-tie-formation-prob              ;; 0.05 per session (Ryan 2011)
...
to setup-parks-pilot
  create-parks num-parks [
    ...
    set has-indoor-partner? (random-float 1.0 < 0.8)
...
to recruit-refugees-pilot
  ;; Target: 6-9 refugees per park (use slider refugees-per-group)
  ...
  create-refugees target-count [
    ...
    set has-childcare? (random-float 1.0 < 0.4)  ;; Only 40% initially
    set can-access-transport? (random-float 1.0 < 0.7)  ;; 70% have access
...
;; ----------------------------------------------------------------------------
;; Recruit Locals (≤3 per park)
;; ----------------------------------------------------------------------------

to recruit-locals-pilot
  ;; Target: ≤3 locals per park
  ...
  create-locals target-count [
    ...
    set has-childcare?         (random-float 1.0 < 0.80)  ;; 80% have childcare sorted
    set receives-stipend?      false                       ;; locals never receive stipend
    set can-access-transport?  (random-float 1.0 < 0.90)  ;; 90% have transport access
...
;; ----------------------------------------------------------------------------
;; Form Training Groups (target composition: 1:≤3:6-9)
;; ----------------------------------------------------------------------------
...
to form-training-groups
  ...
  ask trainers [
    ...
    ;; Check optimal composition (1:≤3:6-9)
    set group-composition-optimal? (
      current-group-size >= 9 and
      current-group-size <= 12 and
      locals-in-group >= 2 and
      locals-in-group <= 3 and
      refugees-in-group >= 6 and
      refugees-in-group <= 9
    )
  ]
end
...
to initialize-friendship-networks
  ...
  ask refugees [
    ...
    if count same-park-people > 0 and random-float 1.0 < 0.2 [
  ...
  ask locals [
    ...
    if count same-park-people > 0 and random-float 1.0 < 0.15 [
      let new-friend one-of same-park-people
      if new-friend != nobody and not friendship-neighbor? new-friend [
        create-friendship-with new-friend [
          set tie-strength 0.2
...
to apply-scenario-configuration
  ...
  if scenario-type = "Minimal Support" [
    ...
    ask refugees [
      ...
      set has-childcare? (random-float 1.0 < 0.4)  ;; Reset to baseline 40%
    ]
  ]
  ...
  if scenario-type = "Weak Peer Influence" [
    ...
    set param-peer-influence-coef 0.03  ;; vs baseline 0.08
  ]
  ...
  if scenario-type = "NoIndoor Minimal" [
    ...
    ask refugees [
      set receives-stipend? false
      set has-childcare? (random-float 1.0 < 0.4)
    ]
  ]
...
to decide-weekly-attendance-refugee  ;; refugee procedure
  ...
  if gender = "female" and current-season = "outdoor" and random-float 1.0 < 0.2 [
    set base-prob base-prob * 0.9  ;; 10% reduction
  ]
...
to conduct-training-sessions
  ...
  repeat sessions-per-week [
    ask (turtle-set refugees locals) with [current-week-attendance] [
      ...
      set motivation min (list 1.0 (motivation + 0.03))
...
to update-friendship-networks
  ask friendships [
    ...
    ifelse both-active? [
      ;; Strengthen tie
      set tie-strength min (list 1.0 (tie-strength + 0.03))
...
to language-learning-from-sessions  ;; refugee procedure
  ...
  ;; Logistic ceiling factor: diminishing returns near C2
  ...
  ask (turtle-set refugees locals) with [not has-dropped-out? and not winter-paused?] [
    ...
    if motivation < param-dropout-threshold and not current-week-attendance [
      ...
      if motivation < adjusted-threshold [
        if random-float 1.0 < 0.2 [  ;; 20% chance per week below threshold
    ...
    if work-hours-conflict? and not receives-stipend? and not current-week-attendance [
      let work-dropout-prob ifelse-value (breed = refugees) [0.02] [0.01]
    ...
    if current-season = "indoor" [
      ...
      if my-park-agent != nobody and not [has-indoor-partner?] of my-park-agent [
        if random-float 1.0 < 0.05 [  ;; 5% chance per winter week → winter pause
    ...
    if distance-to-park > 5 and not can-access-transport? and not current-week-attendance [
      let dist-dropout-prob ifelse-value (breed = refugees) [0.03] [0.015]
...
to check-spring-reentry
  ...
  if current-season = "outdoor" [
    ask (turtle-set refugees locals) with [winter-paused? and not has-dropped-out?] [
      ...
      let reentry-prob min (list 0.70 (0.25 + total-friend-count * 0.06))
...
scenario-type
scenario-type
"Baseline" "No Indoor Continuity" "Minimal Support" "Low Park Density" "Weak Peer Influence" "Suboptimal Composition" "High SES Heterogeneity" "Women-Only Groups" "NoIndoor Minimal" "Targeting50" "Targeting70" "Targeting90" "BuddyProgram" "RotatingGroups" "Winter50" "WomenChildcare" "Equifinality"
0
...
The **Calisthenics Integration Model (CIM) v6.3** is an agent-based model that explores how structured community sport programmes can help migrants integrate into a host society. Specifically, it simulates a one-year calisthenics training programme where migrants and locals train together in public parks.
...
### All 16 Scenarios (see Table 3.3 in thesis)

**Original hypothesis tests (8 scenarios):**
...
**New v6.3 scenarios (8 scenarios):**
...
BuddyProgram       Social        Migrant paired with local buddy for 8 weeks
...
### Key Parameters (see Table 3.1 in thesis)

```
Parameter                      Default  Range           Source
---------                      -------  -----           ------
motivation-decay-rate          0.018    [0.005, 0.040]  Exercise adherence
peer-influence-coefficient     0.080    [0.010, 0.150]  Social contagion
...
- **The BuddyProgram and HighSES**: BuddyProgram (48.3%) and High SES Heterogeneity (48.5%) both beat Baseline (44.3%). WomenChildcare (45.6%) also outperforms Baseline
...
3. Test **BuddyProgram** - does the buddy pairing keep migrants motivated through winter?
4. Compare **Women-Only Groups** vs **WomenChildcare** - does adding childcare close the gender dropout gap?
5. Run the three **Targeting** scenarios (50/70/90) - at what accuracy does targeting recover Baseline performance?
...
- Gjestvang et al. (2020): 12-month exercise adherence rates
...
**Full documentation**: See the accompanying thesis (70 pages) and the R analysis pipeline (19 scripts) for complete methodology, validation, and results.
